...
QByteArray QAESEncryption::Encrypt(QAESEncryption::Aes level,
                                   QAESEncryption::Mode mode,
                                   const QByteArray &rawText,
                                   const QByteArray &key, const QByteArray &iv,
                                   QAESEncryption::Padding padding) {
    return QAESEncryption(level, mode, padding).encrypt(rawText, key, iv);
}
...
QByteArray QAESEncryption::Decrypt(QAESEncryption::Aes level,
                                   ...
                                   const QByteArray &key, const QByteArray &iv,
                                   QAESEncryption::Padding padding) {
    return QAESEncryption(level, mode, padding).decrypt(rawText, key, iv);
}
...
#else
            ret.remove(ret.length() - ret.at(ret.length() - 1),
                       ret.at(ret.length() - 1));
#endif
            break;
        case Padding::ISO: {
...
QByteArray QAESEncryption::expandKey(const QByteArray &key) {
    ...
    for (i = m_nk; i < m_nb * (m_nr + 1); i++) {
        ...
        roundKey.insert(i * 4 + 0,
                        (quint8)roundKey.at((i - m_nk) * 4 + 0) ^ tempa[0]);
        roundKey.insert(i * 4 + 1,
                        (quint8)roundKey.at((i - m_nk) * 4 + 1) ^ tempa[1]);
        roundKey.insert(i * 4 + 2,
                        (quint8)roundKey.at((i - m_nk) * 4 + 2) ^ tempa[2]);
        roundKey.insert(i * 4 + 3,
                        (quint8)roundKey.at((i - m_nk) * 4 + 3) ^ tempa[3]);
    }
    return roundKey;
}
...
void QAESEncryption::addRoundKey(const quint8 round, const QByteArray &expKey) {
    ...
    for (int i = 0; i < 16; ++i)
        it[i] = (quint8)it[i] ^
                (quint8)expKey.at(round * m_nb * 4 + (i / 4) * m_nb + (i % 4));
}
...
void QAESEncryption::mixColumns() {
    ...
    for (int i = 0; i < 16; i += 4) {
        t = (quint8)it[i];
        tmp = (quint8)it[i] ^ (quint8)it[i + 1] ^ (quint8)it[i + 2] ^
              (quint8)it[i + 3];
...
void QAESEncryption::invMixColumns() {
    ...
    for (int i = 0; i < 16; i += 4) {
        ...
        it[i] = (quint8)(multiply(a, 0x0e) ^ multiply(b, 0x0b) ^
                         multiply(c, 0x0d) ^ multiply(d, 0x09));
        it[i + 1] = (quint8)(multiply(a, 0x09) ^ multiply(b, 0x0e) ^
                             multiply(c, 0x0b) ^ multiply(d, 0x0d));
        it[i + 2] = (quint8)(multiply(a, 0x0d) ^ multiply(b, 0x09) ^
                             multiply(c, 0x0e) ^ multiply(d, 0x0b));
        it[i + 3] = (quint8)(multiply(a, 0x0b) ^ multiply(b, 0x0d) ^
...
QByteArray QAESEncryption::cipher(const QByteArray &expKey,
                                  const QByteArray &in) {
...
QByteArray QAESEncryption::invCipher(const QByteArray &expKey,
                                     const QByteArray &in) {
...
QByteArray QAESEncryption::encrypt(const QByteArray &rawText,
                                   const QByteArray &key,
                                   const QByteArray &iv) {
    ...
    switch (m_mode) {
        ...
        case CBC: {
            ...
            for (int i = 0; i < alignedText.size(); i += m_blocklen) {
                alignedText.replace(
                    i, m_blocklen,
                    byteXor(alignedText.mid(i, m_blocklen), ivTemp));
                ret.append(cipher(expandedKey, alignedText.mid(i, m_blocklen)));
        ...
        case CFB: {
            QByteArray ret;
            ret.append(
                byteXor(alignedText.left(m_blocklen), cipher(expandedKey, iv)));
            for (int i = 0; i < alignedText.size(); i += m_blocklen) {
                if (i + m_blocklen < alignedText.size())
                    ret.append(
                        byteXor(alignedText.mid(i + m_blocklen, m_blocklen),
...
QByteArray QAESEncryption::decrypt(const QByteArray &rawText,
                                   const QByteArray &key,
                                   const QByteArray &iv) {
    if (m_mode >= CBC && (iv.isEmpty() || iv.size() != m_blocklen))
        return QByteArray();
    ...
    switch (m_mode) {
        ...
        case CBC: {
            ...
            for (int i = 0; i < rawText.size(); i += m_blocklen) {
                ret.append(invCipher(expandedKey, rawText.mid(i, m_blocklen)));
                ret.replace(i, m_blocklen,
                            byteXor(ret.mid(i, m_blocklen), ivTemp));
                ivTemp = rawText.mid(i, m_blocklen);
            }
        } break;
        case CFB: {
            ret.append(
                byteXor(rawText.mid(0, m_blocklen), cipher(expandedKey, iv)));
            for (int i = 0; i < rawText.size(); i += m_blocklen) {
                if (i + m_blocklen < rawText.size()) {
                    ret.append(byteXor(
                        rawText.mid(i + m_blocklen, m_blocklen),
